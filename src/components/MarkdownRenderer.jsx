...
export default function MarkdownRenderer({ content, className = '' }) {
  const htmlContent = useMemo(() => {
    ...
    html = html.replace(/\$\$([\s\S]*?)\$\$/g, (match, math) => {
      return `<div class="my-4 overflow-x-auto">${renderMath(math.trim(), true)}</div>`
    })
    ...
    html = html.replace(/`([^`]+)`/g, '<code class="bg-primary-100 text-primary-700 px-1.5 py-0.5 rounded text-sm font-mono">$1</code>')
    
    // Headers
    html = html.replace(/^#### (.*$)/gm, '<h4 class="text-base font-bold text-gray-900 mt-5 mb-2">$1</h4>')
    html = html.replace(/^### (.*$)/gm, '<h3 class="text-lg font-bold text-gray-900 mt-5 mb-2">$1</h3>')
    ...
    // Lists - unordered
    html = html.replace(/^\* (.*$)/gm, '<li class="ml-5 list-disc text-gray-700 my-1 pl-1">$1</li>')
    html = html.replace(/^- (.*$)/gm, '<li class="ml-5 list-disc text-gray-700 my-1 pl-1">$1</li>')
    
    // Lists - ordered
    html = html.replace(/^\d+\. (.*$)/gm, '<li class="ml-5 list-decimal text-gray-700 my-1 pl-1">$1</li>')
    ...
    // Blockquotes
    html = html.replace(/^&gt; (.*$)/gm, '<blockquote class="border-l-4 border-primary-500 pl-4 py-2 my-4 bg-primary-50 rounded-r-lg text-gray-700 italic">$1</blockquote>')
    ...
    codeBlocks.forEach(({ lang, code }, id) => {
      ...
      html = html.replace(`__CODE_BLOCK_${id}__`, `<pre class="bg-gray-900 text-gray-100 p-4 rounded-xl overflow-x-auto my-4 text-sm"><code>${escaped}</code></pre>`)
    })
    ...
    html = html.split('\n\n').map(block => {
      ...
      return `<p class="text-gray-700 leading-relaxed my-2">${block}</p>`
    }).join('')
